...
It allows you to accept commas, currency symbols, and percent symbols from users for integer and float column.  You can also set a numeric column to act as a boolean, so that no matter what format the boolean is stored in the database, it is handled correctly.
...
create_table :cities do |t|
  ...
  t.tinyint :historic,                        :default => 0, :null => true
end
...
Historic will work as a boolean, whether it is stored as a 1/0 or a -1/0 or a t/f.  your_city.historic and your_city.historic? will both return true or false appropriately.  The following are false, all else are true:
