...
It allows you to accept commas, currency symbols, and percent symbols from users for integer and float columns.  You can also set a numeric column to act as a boolean, so that no matter what format the boolean is stored in the database, it is handled correctly.
...
Historic, which is set as acts_as_boolean, will work as a boolean, whether it is stored as 1/0 or -1/0 or t/f.  Both methods, historic and historic?, will return the same value; either true or false.  The following are false, all else are true:
